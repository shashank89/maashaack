...
package abcproguard
{
    ...
    class MethodInfo extends MemberInfo
    {
		...
		var initScopeDepth:int;
		var exInfo:Array;
		var optional_count:int;
		var tag:int;
    ...
    class Abc implements ISWFTag
    {
        ...
        function parseMethodInfos()
        {
            ...
            for (var i:int=0; i < method_count; i++)
            {
                ...
                {
                    // has_optional
                    m.optional_count = readU32();
                    m.optionalValues = []
					m.optionalValuesIndex = [];
					m.optionalValuesType = [];
                    for( var k:int = param_count-m.optional_count; k < param_count; ++k)
                    {
                        var index = readU32()    // optional value index
                        var kind:int = data.readByte() // kind byte for each default value
						
						m.optionalValuesIndex[k] = index;
						m.optionalValuesType[k] = kind;
                        if (index == 0)
                        {
        ...
        function parseTraits(t:Traits, debug : Boolean = false)
        {
        ...
        function parseMethodBodies()
        {
            ...
            {
				var index : int = readU32();
				methodsIndex[i] = index
                var m = methods[index]
                m.max_stack = readU32()
                m.local_count = readU32()
                m.initScopeDepth = readU32()
                var maxScopeDepth = readU32()
                m.max_scope = maxScopeDepth - m.initScopeDepth
                ...
                for (var j:int = 0; j < ex_count; j++)
                {
                    ...
                    info.to = readU32()
                    info.target = readU32()
                    info.type = names[readU32()]
                    //print("magic " + magic.toString(16))
                    //if (magic >= (46<<16|16))
                    info.name = names[readU32()];
		...
		public static function writeU32(data : ByteArray, n : int)
		{
			while(true)
			{
				if((n >>> 7) > 0)
				{
					data.writeByte((n & 0x7F) | 0x80)
					n = n >>> 7
				}
				else
				{
					data.writeByte(n & 0x7F)
					break
				}
			}
		}
		
		function getNsIndexByUri(uri : String) : int
		{
			var n : int = namespaces.length;
		...
		public function write(data : ByteArray)
		{
            ...
            writeMethodInfos(data)
            writeMetadataInfos(data)
            writeInstanceInfos(data)
            writeClassInfos(data)
            writeScriptInfos(data)
            writeMethodBodies(data)
		}
		
		function writeCpool(data : ByteArray)
		{
			...
			writeU32(data, n)
			for(i = 1; i < n; ++i)
				writeU32(data, ints[i])
			
            // uints
			n = uints.length
			writeU32(data, n)
			for(i = 1; i < n; ++i)
				writeU32(data, uints[i])
			...
			for(i = 1; i < n; ++i)
			{
				data.writeByte(namespacesKind[i])
                switch (namespacesKind[i])
                {
                case CONSTANT_Namespace:
                case CONSTANT_PackageNs:
		...
		function writeMethodInfos(data : ByteArray)
		{
			...
			var method_count : int = methods.length;
			writeU32(data, method_count);
			for(var i : int = 0; i < method_count; ++i)
			{
				var m : MethodInfo = methods[i];
				var param_count : int = m.paramTypes.length;
				writeU32(data, param_count);
                ...
                {
                    // has_optional
					var optional_count : int = m.optional_count;
					writeU32(data, optional_count);
					
					for(var k : int = param_count - optional_count; k < param_count; ++k)
					{
						writeU32(data, m.optionalValuesIndex[k]);
						data.writeByte(m.optionalValuesType[k]);
					}
				}
                ...
                if (m.flags & HAS_ParamNames)
                {
					...
					for(var k : int = 0; k < param_count; ++k)
					{
						writeU32(data, m.paramNames[k]);
					}
				}
			}
			infoPrint("MethodInfo write size:" + method_count + "/" + (data.position - start));
		}
		
		function writeMetadataInfos(data : ByteArray)
		{
			var start = data.position;
			var count : int = metadata.length;
			writeU32(data, count);
			for(var i : int = 0; i < count; ++i)
			{
		...
		function writeInstanceInfos(data : ByteArray)
		{
			var start = data.position;
			var count : int = instances.length;
			writeU32(data, count);
			for(var i : int = 0; i < count; ++i)
			{
				var t : Traits = instances[i];
				writeU32(data, names.indexOf(t.name));
				writeU32(data, names.indexOf(t.base));
				data.writeByte(t.flags);
				if(t.flags & 8)
					writeU32(data, namespaces.indexOf(t.protectedNs));
				var interface_count : int = t.interfaces.length;
				writeU32(data, interface_count);
				for(var j : int = 0; j < interface_count; ++j)
					writeU32(data, names.indexOf(t.interfaces[j]));
				writeU32(data, methods.indexOf(t.init));
				
				writeTraits(data, t);
			}
			
			infoPrint("InstanceInfo write size:" + count + "/" + (data.position - start));
		}
		
		function writeTraits(data : ByteArray, t : Traits, debug : Boolean = false) : void
		{
			var namecount = t.members.length;
			writeU32(data, namecount);
			for(var i : int = 0; i < namecount; ++i)
			{
				var member : * = t.members[i];
				writeU32(data, names.indexOf(member.name));
				data.writeByte(member.tag);
                ...
                case TRAIT_Const:
                case TRAIT_Class:
                    var slot : SlotInfo = member as SlotInfo;
                    writeU32(data, slot.id);
                    if (member.kind==TRAIT_Slot || member.kind==TRAIT_Const)
                    {
						writeU32(data, names.indexOf(slot.type));
						writeU32(data, slot.valueIndex);
						if(slot.valueIndex)
							data.writeByte(slot.valueType);
                        /*var index=readU32()
                        if (index)
                    ...
                    {
						writeU32(data, classes.indexOf(slot.value));
                    }
                    break;
                case TRAIT_Method:
                case TRAIT_Getter:
				...
				if((member.tag >> 4) & ATTR_metadata)
				{
					var mdCount : int = member.metadata.length;
					writeU32(data, mdCount);
					for(var j : int = 0; j < mdCount; ++j)
					{
						writeU32(data, metadata.indexOf(member.metadata[j]));
					}
				}
		...
		function writeClassInfos(data : ByteArray)
		{
			var start : int = data.position;
			var count : int = instances.length;
			for(var i : int = 0; i < count; ++i)
			{
				var t : Traits = classes[i];
				writeU32(data, methods.indexOf(t.init));
				writeTraits(data, t);
			}
		...
		function writeMethodBodies(data : ByteArray)
		{
			var start : int = data.position;
			var count : int = methodsIndex.length;
			writeU32(data, count);
			for(var i : int = 0; i < count; ++i)
			{
				writeU32(data, methodsIndex[i]);
				var m : MethodInfo = methods[methodsIndex[i]];
				if(!m)
				{
					print(i, methodsIndex[i]);
				}
				writeU32(data, m.max_stack);
				writeU32(data, m.local_count);
				writeU32(data, m.initScopeDepth);
				writeU32(data, m.max_scope + m.initScopeDepth);
				writeU32(data, m.code.length);
				if(m.code.length > 0)
					data.writeBytes(m.code, 0, m.code.length);
				var ex_count : int = m.exInfo.length;
				writeU32(data, ex_count);
				for(var j : int = 0; j < ex_count; ++j)
				{
					var info : Object = m.exInfo[j];
					writeU32(data, info.from);
					writeU32(data, info.to);
					writeU32(data, info.target);
					writeU32(data, names.indexOf(info.type));
					writeU32(data, names.indexOf(info.name));
				}
				
				writeTraits(data, m.activation);
			}
			infoPrint("MethodBodies write size:" + count + "/" + (data.position - start));
		}
    }
